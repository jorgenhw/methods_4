...
coordinates(dag_corona) <- list( 
  x=c(corona_fear =0,social_exposure =-1, sanitizing =0, germ_level=-1, infected= -1), 
  y=c(corona_fear =0,social_exposure =0, sanitizing = 1, germ_level=1, infected=2) )
...
come). (2) Classify each path by whether i t is open or closed. A path is
...
social_exposure <- rbinom(N, size =1, prob=inv_logit(scale(corona_fear)))
...
# creating df
df <- tibble(germ_level, sanitizing, infected, social_exposure, corona_fear) 
```
...
df <- df %>% 
  mutate(germ_std=scale(germ_level),sanitizing_std= scale(sanitizing),  infected_std=scale(infected),social_exposure_std=scale(social_exposure), corona_fear_std= scale(corona_fear))
...
Testing whether the causal coefficient bInfected is insignificant when stratifying by social_exposure and sanitizing. 
```{r}

m3 <- quap(
    alist(
        corona_fear_std ~ dnorm( mu , sigma ) ,
        mu <- a + bInfected * infected_std  + bFaceMask * social_exposure_std + bSanitizing * sanitizing_std,
        a ~ dnorm( 0 , 0.2 ) ,
        bInfected ~ dnorm( 0 , 1 ),
        bFaceMask ~ dnorm( 0, 1 ),
        bSanitizing~ dnorm( 0 , 1 ),
...
```
As seen in the above plot, bInfected becomes independent (i.e., the model does with almost no uncertainty estimate the coefficient bInfected to be very close to zero) when stratifying by social_exposure and sanitizing. 


Model 4: fc_m _||_ infc_ | grm_
Testing whether the causal coefficient infc (being infected or not) significantly close to zero when stratifying by germ_level in a model that has social_exposure as outcome variable.
```{R} 
...
coordinates(twisted_dag_corona) <- list( 
  x=c(corona_fear =0, OCD = .3, social_exposure =-.5, sanitizing =0, germ_level=-.5, infected= -.5) , 
  y=c(corona_fear =0, OCD = .5, social_exposure =0, sanitizing = 1, germ_level=1, infected=2) )
...
# creating df
df_w <- tibble(germ_level_w, sanitizing_w, infected_w, social_exposure_w, corona_fear_w, OCD_w) 
...
df_w <- df_w %>% 
  mutate(germ_std_w=scale(germ_level_w),sanitizing_std_w= scale(sanitizing_w),  infected_std_w=scale(infected_w),social_exposure_std_w=scale(social_exposure_w), corona_fear_std_w= scale(corona_fear_w), OCD_std_w = scale(OCD_w))
...
model1_w <- quap(
    alist(
        ...
        mu <- a + bFaceMask*social_exposure_std_w + bCoronaFear*corona_fear_std_w,
        a ~ dnorm( 0 , 0.2 ) ,
...
model2_w <- quap(
    alist(
        OCD_std_w ~ dnorm( mu , sigma ) ,
        mu <- a + bGermLevel*germ_std_w + bFaceMask*social_exposure_std_w + bSanitize*sanitizing_std_w ,
        a ~ dnorm( 0 , 0.2 ) ,
...
model5_w<- quap(
    alist(
        OCD_std_w ~ dnorm( mu , sigma ) ,
        mu <- a + bInfected*infected_std_w  + bsocial_exposure*social_exposure_std_w + bSanitizing*sanitizing_std_w,
        a ~ dnorm( 0 , 0.2 ) ,
        bInfected~ dnorm( 0 , 1 ) ,
...
model6_w<- quap(
    alist(
        OCD_std_w ~ dnorm( mu , sigma ) ,
        mu <- a + benergylvl*infected_std_w  + bCorona_fear*corona_fear_std_w + bSanitizing*sanitizing_std_w,
        a ~ dnorm( 0 , 0.2 ) ,
        bInfected~ dnorm( 0 , 1 ) ,
