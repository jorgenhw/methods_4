...
OCD is a backdoor that we should control for (i.e., include in a our model) in order to find the full causal effect of corona fear on infected. 
...
face_mask <- rbinom(N, size =1, prob=inv_logit(scale(corona_fear*2)))
...
sanitizing <-  rnorm(N, mean= (corona_fear-9)+(OCD*2), sd=2)
hist(sanitizing)
...
#infected <- rnorm(N, mean=-0.8*germ_level+4200, sd = 2)
plot(infected,germ_level)
hist(infected)
hist(germ_level)
...
Testing whether the causal coefficient of face_mask becomes significantly close to zero when stratifying by corona fear in a model that has OCD as outcome variable.
```{r}
m7 <- quap(
    alist(
        OCD ~ dnorm( mu , sigma ) ,
...
precis(m8)
```
(OBS) Chris: coefficient is very low - but why not almost zero? Is it the model (should we have different slopes/intercepts dependent on our categorical predictor face_mask) or does it have something to do with our data generation process???
...
Testing whether the causal coefficient of infected becomes significantly close to zero when stratifying by germ level in a model that has OCD as outcome variable.
```{r}
m10<- quap(
    alist(
        OCD ~ dnorm( mu , sigma ) ,
...
```
As seen in the above plot, bInfected becomes independent from OCD (i.e., the model does with almost no uncertainty estimate the coefficient bInfected to be very close to zero) when stratifying by germ level. 



Model 11: OCD _||_ infc | fc_m, sntz
Testing whether the causal coefficient of infected becomes significantly close to zero when stratifying by face mask and sanitizing in a model that has OCD as outcome variable.
```{r}
...
```
As seen in the above plot, bInfected becomes independent from OCD (i.e., the model does with almost no uncertainty estimate the coefficient bInfected to be very close to zero) when stratifying by face mask.


Model 12: OCD _||_ infc | crn_, sntz 
Testing whether the causal coefficient of infected becomes significantly close to zero when stratifying by corona fear and sanitizing in a model that has OCD as outcome variable.
```{r}
m12<- quap(
    alist(
        OCD ~ dnorm( mu , sigma ) ,
...
```
As seen in the above plot, bInfected becomes independent from OCD (i.e., the model does with almost no uncertainty estimate the coefficient bInfected to be very close to zero) when stratifying by sanitizing and corona fear. 
...
m_no_backdoor<- quap(
    alist(
        infected_std ~ dnorm( mu , sigma ) , # (OBS ask Chris about standardizing dichotomous variable)
        mu <- a + bCorona_fear*corona_fear_std + bOCD*OCD,# (OBS ask Chris about standardizing dichotomous variable)
        a ~ dnorm( 0 , 0.2 ) ,
        bCorona_fear~ dnorm( 0 , 1 ),
        bOCD~ dnorm( 0 , 1 ),
        sigma ~ dexp( 1 )
