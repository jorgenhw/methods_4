...
Testing whether the causal coefficient of energy_level becomes significantly close to zero when stratifying by germ level in a model that has OCD as outcome variable. OBS! Categorical outcome?

```{r}

adjustmentSets(dag_corona,exposure="OCD", outcome="energylvl") # crn_ _||_ enrg | grm_ 

m1 <- quap(
    alist(
        OCD_std ~ dnorm( mu , sigma ) ,
        mu <- a + bEng*energylvl+bGermlvl*germ_std,
        a ~ dnorm( 0 , 0.2 ) ,
        bEng ~ dnorm( 0 , 1) ,
        bGermlvl ~ dnorm( 0 , 1) ,
        sigma ~ dexp( 1 ) 
...
```
As seen in the above plot, XXXX  becomes independent from XXXX (i.e., the model does with almost no uncertainty estimate the coefficient XXXX to be very close to zero) when stratifying by XXXX.
...
Testing whether the causal coefficient of energy level becomes significantly close to zero when stratifying by sanitizing and social exposure in a model that has OCD as outcome variable. OBS HELP?
```{r}
m2 <- quap(
    alist(
        OCD ~ dnorm( mu , sigma ) ,
...
Model 11: OCD _||_ infc | enrg, sntz
...
Model 1: crn_ _||_ grm_ | enrg, sntz. 
...
Model 3: crn_ _||_ infc_ | enrg, sntz
